...
function normalizedCandidate(note: VaultNote, reference: AttachmentReference): string {
  ...
  if (reference.kind === "obsidian-embed") return path.posix.normalize(target.replace(/^\/+/, ""));
...
export function resolveAttachments(
  ...
  for (const note of index.notes) {
    for (const reference of note.attachments) {
      ...
      if (!entry && reference.kind === "obsidian-embed" && !reference.target.includes("/")) {
        const matches = filesByName.get(path.posix.basename(candidate)) ?? [];
        if (matches.length === 1) entry = matches[0];
        else if (matches.length > 1) {
          diagnostics.push(
            `${note.vaultPath}: ${reference.raw} is ambiguous; matches ${matches.map((match) => match.path).join(", ")}`,
          );
          continue;
        }
      }
      ...
      if (!entry) {
        diagnostics.push(
          `${note.vaultPath}: ${reference.raw} is ${unavailableReason(manifest, candidate)} (${candidate})`,
        );
      } else if (entry.kind === "file") {
        resolved.push({ source: note, reference, entry });
      }
    }
  }
