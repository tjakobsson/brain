...
The site SHALL use one top-right navigation pill as its primary navigation on desktop and smaller screens. In workspace mode, the brain context control MUST be contained within that same pill and appear before Graph, Search, and the expand action. The context control and navigation actions MUST remain available without changing header modes in response to page scrolling, and the unified pill MUST avoid overlapping or hiding page content or its own controls.

#### Scenario: Navigate on desktop
- **WHEN** a reader opens any site page in a desktop viewport
- **THEN** one top-right navigation pill is present instead of a full row of text navigation or a separate brain-selector pill
...
- **THEN** the brain context control and navigation actions remain usable in one pill without overlapping page content or hiding one another

#### Scenario: Read navigation order
- **WHEN** a reader encounters the navigation pill in workspace mode
- **THEN** the brain context control precedes the Graph, Search, and expand actions in visual and focus order
