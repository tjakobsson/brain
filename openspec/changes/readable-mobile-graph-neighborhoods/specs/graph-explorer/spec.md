...
Label collision checks MUST also exclude overlaps with other visible node markers, using their rendered sizes at the current camera state rather than raw graph-space sizes. Label selection on both graphs MUST refresh when hover preview ends or is toggled, without waiting for a camera or pointer change.
