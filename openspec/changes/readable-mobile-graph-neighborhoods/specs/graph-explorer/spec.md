...
- **THEN** the title wraps onto up to three centred lines below its node and no line is cut off by an edge of the graph area
...
- **WHEN** a reader with hover preview on hovers a node
...
Secondary focused-neighborhood information SHALL include the complete titles of the focused note's directly connected visible neighbors as a list of text rows, so a reader can read them without zooming or panning the canvas. The list MUST appear wherever the focused-neighborhood bar's secondary information appears, on both narrow and wide viewports. Titles MUST NOT be shortened; a title too wide for one line MUST wrap. The list MUST be ordered alphabetically by title, MUST include every directly connected neighbor the current filters, lens and context leave visible, and MUST NOT be capped, scrolling within the bar's existing height limit instead. In workspace mode, a neighbor owned by another Brain MUST be identifiable as foreign regardless of the reader's label preference. Each row MUST be an interactive control that moves graph focus to that neighbor, with a touch target of at least 44 by 44 CSS pixels on narrow and coarse-pointer layouts. Moving focus from a row MUST leave the secondary information expanded and refill the list with the newly focused note's neighbors. On a note-owned neighborhood page, where focus identity is the page itself, a row MUST instead navigate to that neighbor's neighborhood page. When the focused note has no visible connected neighbors, the list MUST be absent rather than empty.

#### Scenario: Read neighbor titles without zooming
- **WHEN** a touch reader pins a neighborhood on a phone and expands the focus bar
...
- **THEN** the browser opens that neighbor's own neighborhood page
...
- **THEN** the first painted frame frames the focused note and its direct neighbors, and no zoom away from the whole graph precedes it
