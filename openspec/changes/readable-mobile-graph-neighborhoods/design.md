...
A label that inspection or search styling clears is still on its way out if it was drawn a moment ago. Both graphs keep the last drawn text and layout per label and draw a retiring label from that record until its fade-out finishes, on the final pass only, so it never re-enters the candidate set. A selection change inside another change's fade window keeps the earlier leavers retiring rather than replacing the set with the newest ones; a leaver the new selection takes back fades in from where it is. A label that is only fading out is not a pointer or touch target: it is marked as leaving and hit-testing ignores it, so a reader cannot open a note through a title that is disappearing, and a touch landing where such a title was reaches the marker behind it instead. When a fade-out finishes, only the retired nodes are repainted; a full re-index in the middle of a press or pinch is avoided.
