...
On a phone the focus bar's connected-notes panel opens above the bar as an absolutely positioned child, so the bar's own rectangle does not include it. The fit's bottom inset measures the visible panel's top edge as well as the bar's, so the refit that opening the disclosure triggers keeps the neighborhood's markers above the panel rather than behind it.
