...
- **WHEN** a reader opens Graph from an Engineering note reached through an unpinned Engineering and Design combined view
...
- **THEN** the same pill expands in place and directly reveals every destination applicable to the current page
...
- **WHEN** a keyboard user focuses and activates the launcher, Graph where present, Search, or another navigation destination
