...
- **WHEN** a reader invokes the generator with a readable single brain and writable output directory
- **THEN** the generator writes a complete static site without modifying the source directory
...
- **WHEN** a consumer points the generator at a repository-root brain containing notes, hidden metadata, and `.github`
- **THEN** notes are published while default-excluded metadata and workflow files are omitted
...
**Reason**: The existing requirement defines one scenario through Obsidian terminology and does not define ownership boundaries for multiple brains.

**Migration**: Use the replacement Brain attachment resolution requirement; existing supported attachment syntax retains the same deterministic path and filename behavior inside its owning brain.
...
- **THEN** the generator exits non-zero and lists the conflicting files
