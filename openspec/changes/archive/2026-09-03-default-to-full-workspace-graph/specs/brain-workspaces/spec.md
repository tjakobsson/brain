...
**Reason**: The root page no longer gates the graph behind choosing a Brain. Its identity, hierarchy, and Enter Brain affordances move into the full workspace graph's Brain control, and note ownership and navigation rules are restated in "Full workspace root graph and Brain identity".
**Migration**: Readers land on the full workspace graph; the Brain control lists every configured Brain with a direct action to open it. Workspace-level tags, recent, and orphans pages aggregate all Brains instead of redirecting to a chooser.

### Requirement: Reader-selected combined view
**Reason**: Combined views existed to make a subset of Brains visible and shareable through `?brains=` query context. The full workspace graph now shows every Brain by default, the personal Brain lens covers narrowing without URL state, and the note-owned focused-neighborhood page is the shareable unit. Query-carried selection cannot survive authenticating proxies that return only the pathname.
**Migration**: `/graph` and `?brains=` selections stop resolving to combined views; `/graph` opens the full workspace graph and `?brains=` is ignored. Readers who want fewer Brains dim the others through the Brain control. Shared subsets are replaced by shared neighborhood links.
