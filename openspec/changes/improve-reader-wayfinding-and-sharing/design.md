...
Focus uses the existing inspection reducer for one-hop emphasis, adds a persistent non-color-only marker to the subject, and leaves transient hover higher priority until the pointer exits. On restore, validate that the composite ID belongs to the selected graph, make the subject and direct neighborhood visible despite stored filter history, reveal only directly connected foreign boundary notes when general Related Brains is off, and fit rendered bounds for that neighborhood after layout restoration or initial settling. A later explicit filter or selection that excludes the subject clears focus and canonicalizes the URL.
...
### Make note Graph actions focused by default

The shared navigation Graph action and the note connection map's full-graph link derive one destination from note identity plus browsing scope. A direct note opens its owning Brain graph focused on itself; a note carrying a valid combined scope opens that combined graph focused on itself. This turns the Graph action into a deterministic doorway without changing the note content or adding a selector to the note header.

### Keep single-Brain entry primary and add explicit escape routes

Retain each chooser card's Enter Brain link as its primary action. Relabel card checkboxes as secondary combination controls and keep the fixed combination action absent before any selection. After one selection, reveal guidance to choose another; after two, enable the combined action. Add a Brains destination to workspace navigation so every note, report, and graph can return to the chooser without reintroducing the rejected shared Brain selector.

Add an About icon as the final navigation action. It opens a small bounded disclosure containing selectable `Brain v<semantic-version>` text and, when suitable, a release destination. Inject the same package-derived value into a `<meta name="generator">` element on every page. Do not add timestamps or require repository metadata, which may be absent from released containers and would weaken reproducibility.
...
- [Long expanded navigation can exceed a short viewport] -> Reuse bounded scrolling and verify Brains and About remain reachable with predictable focus order.
