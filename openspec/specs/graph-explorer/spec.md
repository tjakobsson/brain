...
Label collision checks MUST also exclude overlaps with other visible node markers, using their rendered sizes at the current camera state rather than raw graph-space sizes. Label selection on both graphs MUST refresh when hover preview ends or is toggled, without waiting for a camera or pointer change. Label layout measurements MAY be cached, but the cache MUST be bounded so that resting at many distinct zoom levels on a large graph does not retain a layout set per level.
...
#### Scenario: Rest at many zoom levels
- **WHEN** a reader zooms a large graph through many distinct settled camera ratios
- **THEN** the retained label layouts stay bounded to a few zoom levels' worth rather than growing with every level visited

#### Scenario: Toggle preview under a stationary pointer
- **WHEN** a reader turns hover preview on and then off while the pointer rests on the same global-graph node and the camera does not move
- **THEN** the neighbors' titles appear when it turns on and the normal selection returns when it turns off, without waiting for the pointer or camera to move
...
- **THEN** the Hover preview control stays visible and usable within the layout, while a touch reader at any width does not see it
...
A rendered canvas label SHALL be positioned centred horizontally on its node and below the node's marker, and MUST wrap onto at most three lines at word boundaries so that no line exceeds the label's available width. A label whose text still does not fit three lines MUST be shortened with a trailing ellipsis, and MUST be omitted when not even a shortened first line fits. A label's available width MUST NOT depend on where its node sits, so the same title keeps the same shape wherever it is on screen and panning never reflows it; a label near an edge is clipped by that edge rather than rewrapped to avoid it. Labels arriving in or leaving the rendered set MUST change opacity over time rather than appearing and disappearing between frames, on the global graph and on note-page connection maps alike, whether the change comes from the camera or from inspection beginning or ending. A title that inspection removes MUST leave the same way, drawn as it was until its fade ends; a selection change during another change's fade MUST NOT cut the earlier leavers short; and a title that is only fading out MUST NOT be a pointer or touch target. Meeting these constraints MUST NOT change the camera scale or any graph-space node position. At a fitted narrow-screen overview the focused note SHALL remain identifiable on the canvas by its persistent focus indicator whether or not its label is rendered, and its complete title MUST remain available through the focused-neighborhood bar.

#### Scenario: Begin inspection without a cut
- **WHEN** inspection begins on either graph and clears the titles outside the lit neighborhood
- **THEN** those titles are drawn as they were while they fade out, and none of them is a target for a click, hover, or touch while leaving

#### Scenario: Change selection twice inside one fade
- **WHEN** a second selection change arrives while titles from the first are still fading out and does not select them
- **THEN** they continue their fade rather than disappearing in one frame, and a title the second change does select fades back in from where it is

#### Scenario: Fade a connection-map selection change
- **WHEN** hover preview begins or ends on a note-page connection map without the camera moving, so that some titles leave the rendered set and others arrive
- **THEN** leaving titles stay drawn while they fade out and arriving titles fade in, and no title changes between fully drawn and absent in a single frame
...
#### Scenario: Clear focus while the neighborhood is still settling
- **WHEN** a reader clears focus on a neighborhood page before its initial layout and camera fit have finished
- **THEN** that fit is cancelled rather than finishing into the graph's own scope, the graph settles and saves as an overview, and a reload restores the overview rather than the cleared neighborhood's close-up

#### Scenario: Leave before the replacement settle finishes
- **WHEN** a reader clears focus while the neighborhood is still settling and then reloads or leaves before the graph's own settle has finished
- **THEN** nothing half-settled is saved under the graph's own scope, and the next visit settles the graph afresh as an overview

#### Scenario: Come back to a page hidden mid-settle
- **WHEN** a reader clears focus while the neighborhood is still settling, leaves in a way that keeps the page for Back, and returns to it
- **THEN** the graph settles its own scope on being shown again, and nothing half-settled was saved meanwhile

#### Scenario: Come back to a moved pin hidden mid-settle
- **WHEN** a reader moves focus to a neighbor while the neighborhood is still settling, leaves in a way that keeps the page for Back, and returns
- **THEN** the pin is still on the neighbor and the graph settles and refits around it, rather than the camera framing the whole graph with the pin still active

#### Scenario: Move focus while the neighborhood is still settling
- **WHEN** a reader moves focus to a neighbor before the initial settle has finished and later clears focus without any motion in flight
- **THEN** the move fits the new neighborhood as usual and the later clear neither re-lays out the graph nor moves the camera
...
- **THEN** the camera fits the focused note and its visible neighbors within the selected reach rather than every lower-emphasis orientation marker

#### Scenario: Clear an invalidated focus
- **WHEN** the reader applies a type, status, or tag filter that excludes the focused note
- **THEN** Brain clears the focus indicator and returns the address to the graph page's own path
...
#### Scenario: Narrow a wide graph before choosing
- **WHEN** a reader who has not set the preference opens the workspace graph wide and then narrows it below the phone breakpoint, or the reverse
- **THEN** owner identity in labels follows the new layout's default and the control reflects it, and nothing is written to the reader's browser

#### Scenario: Hold a choice storage could not keep
- **WHEN** a reader sets the preference in a browser whose storage is unavailable or rejects the write, and then crosses the breakpoint
- **THEN** the labels and control keep the reader's choice rather than the layout default
